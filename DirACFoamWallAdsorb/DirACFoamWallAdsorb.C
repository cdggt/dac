...
namespace solvers
{
    defineTypeNameAndDebug(DirACFoamWallAdsorb, 0);
    addToRunTimeSelectionTable(solver, DirACFoamWallAdsorb, fvMesh);
}
}
...
void Foam::solvers::DirACFoamWallAdsorb::correctCoNum()
{
    fluidSolver::correctCoNum(phi);
}

// Used to set maximum time step based on diffusion
void Foam::solvers::DirACFoamWallAdsorb::correctDiNum()
{
...
void Foam::solvers::DirACFoamWallAdsorb::continuityErrors()
{
...
Foam::solvers::DirACFoamWallAdsorb::DirACFoamWallAdsorb(fvMesh& mesh)
:
    fluidSolver(mesh),
...
//(I did not touch anything else)
{
    ...
    if (transient())
    {
        ...
        dCO2aAvg = maxCO2aDelta;
        dCO2aNum = maxCO2aDelta;
    }
    else if (LTS)
    {
        Info<< "Using LTS" << endl;
...
Foam::solvers::DirACFoamWallAdsorb::~DirACFoamWallAdsorb()
{}


// * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * * //

void Foam::solvers::DirACFoamWallAdsorb::readControls()
{
    maxDi = runTime.controlDict().lookupOrDefault<scalar>("maxDi", 1.0);

    maxCo = runTime.controlDict().lookupOrDefault<scalar>("maxCo", 1.0);

    maxCO2Delta = runTime.controlDict().lookupOrDefault<scalar>("maxCO2Delta", 1.0);

    maxCO2aDelta = runTime.controlDict().lookupOrDefault<scalar>("maxCO2aDelta", 1.0);
...
Foam::scalar Foam::solvers::DirACFoamWallAdsorb::maxDeltaT() const
{
...
void Foam::solvers::DirACFoamWallAdsorb::preSolve()
{
    // Read the controls
    readControls();
...
void Foam::solvers::DirACFoamWallAdsorb::prePredictor()
{
...
void Foam::solvers::DirACFoamWallAdsorb::pressureCorrector()
{
...
void Foam::solvers::DirACFoamWallAdsorb::postCorrector()
{
...
void Foam::solvers::DirACFoamWallAdsorb::postSolve()
{}
